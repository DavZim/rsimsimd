...
packages <- subset(packages,
                   sapply(packages$authors, length) > 0 & name != "rsimsimd")
authors <- vapply(packages$authors,
...
writeLines(c("Authors of vendored cargo crates", lines, footer),
           "../../inst/AUTHORS")
cat("Finished writing authors to inst/AUTHORS")
